...
2. 翻訳
   - DeepL API / 各クラウド翻訳 / ローカル CTranslate2 モデルの複数バックエンドをサポート。
...
- ローカル推奨: faster-whisper, ctranslate2, nvidia-cublas-cu12 / nvidia-cudnn-cu12（GPU 利用時）
...
- PyInstaller spec (`backend.spec`) の `datas` に `./src-python/models/translation/prompt` および `./src-python/models/translation/languages` を追加。
