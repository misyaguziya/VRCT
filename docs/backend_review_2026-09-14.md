...
| P-9 | 結果がまとめてドッと出る（1回の呼び出しで ASR を複数回まわす） | +10 行 / 1 ファイル | 🟢 `4bb8cbc6` |
| P-5 | 複数ターゲット言語の翻訳を並列化 | — | ⛔ **実機で回帰、取り消し済み `afa73518`** |
...
### ⛔ P-5 複数ターゲット言語が逐次翻訳される — **並列化は採れない。取り消し済み（`afa73518`）**

> **実機で明確な回帰**。`ThreadPoolExecutor` による並列化（`73366d0a`）を入れたところ、
> 35 秒フリーズが発生し `freeze_trace.log` に記録された。**この方式は採用できない。**
