...
また、前回「大規模構造変更なので着手前に個別スコープ相談が必要」としていた項目 33〜36 の 4 つは、**現在のコードを実測した結果、4 つとも不要**と判定を改める（後述「対応不要と判断した項目」）。
（当初は項目 36 を「S-4 で完了」としていたが、**S-4 は実装すると挙動が退行するため着手しない**と再判定した。）

> **2026-09-15 追記**: 上記 2.（コピペ重複）は、**S-1 を巻き戻し、S-3 を着手しないと判定した**結果、
> 残っていない。着手条件は「実際に起きた事故があるか」「grep / 追跡可能性が改善するか」であって、
> 削減行数ではない。詳細は「レビュー結果（2026-09-15）」を参照。
...
| S-2 | UI 契約の整合テスト 1 本 | 新規 110 行 | 🟢 `ff784899`（実在の不整合を 1 件検出） |
| S-3 | トグル 23・数値 setter 12・委譲 35 のテーブル化 | −300 行 / 1 ファイル | ⛔ **着手しない**（下記） |
...
| S-4 | `models/obs` の config 依存を注入に | 30 行 / 3 ファイル | ⛔ **着手しない**（挙動が退行する。下記） |
...
### P-8 / P-9 は当初 VAD 経路に入っていなかった（P-9 は `f5e2e2f3`、P-8 は `475e3634` で是正）

> **この節自体が一度間違っていた。** 当初「P-8 / P-9 とも 2026-09-15 に是正」と書いたが、
> 実際に VAD 経路へ入れたのは **P-9 だけ**で、P-8（ドレイン単位の畳み込み）は
> 入っていなかった。レビュー（2026-09-15、下記「レビュー結果」）で指摘され `475e3634` で是正。
> **「両方の経路を確認した」と書いた記述自体が未確認だった**という、同じ失敗の二重化。
...
P-8 の方は `475e3634` で是正。VAD 側は `elif is_google:`、非 VAD 側は
`elif is_google and audio_queue.empty():` と、ガードの有無だけが違っていた。
**テストも食い違いを固定化していた**: 同じ「2 チャンクがバックログ」の状況に対し、
非 VAD 版は `call_count == 1`（「中間結果の分まで ASR を呼んではいけない」）、
VAD 版は `call_count == 2` を正としていた。テストが両方あっても、
**期待値が食い違っていれば差異は検出されない**。
...
### ⛔ P-6 `Clipboard` の監視スレッドが停止されない — **着手しない**（2026-09-15 のレビューで判定）

> **判定: P-4 と同じ失敗の形。`Clipboard.stop()` は下記の窓を 1 ミリ秒も閉じない。**
>
> `_stop_monitoring` が読まれるのは `_monitor_steamvr` の `while` の**先頭だけ**である。
> 下の「実害」が挙げる `openvr_session.acquire()` は `_setup_vr_app_name()` の中、
> `clipboard.py:226` の `acquire()` と `:255` の `finally: release()` の間にある。
> **スレッドが既にその区間に入っていたら、フラグを True にしても何も起きない。**
> `time.sleep(10)` 中ならそもそも参照を持っていない。
> 効果があるのは「10 秒以内に来る*未来の* acquire を止める」ことだけで、
> その 10 秒後にはプロセスはもう終了している。
>
> P-4 の教訓そのもの: **「停止処理を呼ぶ」だけでは不十分で、その停止処理自体が
> 制限時間内に目的を達成できるか**まで確認する必要がある。
> しかも `_stopServiceForShutdown` は 5 秒の join 付きなので、`stop()` が join まで行う実装にすると
> `time.sleep(10)` を中断できず、**P-4 の初回修正と同じ 5 秒の終了遅延を作り込む**。
>
> **着手してよい条件**: 実機ログに「終了後も OpenVR が解放されていない」が実際に出た場合のみ。
> そのときの正しい修正箇所は `Clipboard` ではなく **`models/openvr_session`**
> （Overlay と Clipboard の両方がここを通る唯一の合流点）で、shutdown 後の `acquire()` を
> 拒否する形。呼び出し元ごとにフラグを足すのは、P-8 / P-9 が VAD 経路に入っていなかったのと
> 同じ「片方だけ直る」パターンになる。

#### 元の指摘（参考）— P-6 `Clipboard` の監視スレッドが停止されない
...
### 🟢 S-2 フロント / バックエンドのエンドポイント契約を検証するものが何も無い（`ff784899`）
...
**実装**: `src-python/test/test_ui_endpoint_contract.py`。
`ui_config_setter.js` を正規表現で読み、`logics_template_id` から期待エンドポイントを導出して
`mainloop.mapping` に在ることを assert する。型生成器やビルドステップは追加していない。

**当初の仕様のままでは成立しなかった。** `get_set` から `/set/data/<name>` を導出すると、
**意図的な偽陽性が 15 件**出る（`selectable_*_list` 系 13 本、`websocket_auth_token`、
`ollama_url`）。`get_set` は「読み取り専用だが get は欲しい」項目にも流用されており、
`websocket_auth_token` には JS 側にその旨のコメントもある。
そこで実測で 100% 成立している規則だけに絞った:

| 宣言 | 導出するエンドポイント |
|---|---|
| 全エントリ（`weight_download_status` を除く） | `/get/data/<name>` |
| `toggle_enable_disable` | `/set/enable/<name>` と `/set/disable/<name>` の**両方** |
| `get_set_delete` | `/delete/data/<name>` |
| `weight_download_status` | `/run/download_<name>` |
| — | `/set/data/<name>` は**検査しない** |

パース前提（各エントリが 2 つのキーをこの順で持つ）自体を検証するテストも置いた。
前提が崩れたときに黙って素通りしないため。

**実在の不整合を 1 件検出した**（このテストの着手根拠）:
`src-ui` の `OllamaURL` 宣言は 2025-11-14 に LMStudio ブロックのコピペで入ったが、
...
### ⛔ S-3 機械的に集約できるパターンが 3 系統残っている — **着手しない**（2026-09-15 のレビューで判定）

> **判定: 3 系統とも着手しない。** 現物を実測した結果、下記の「対応方針」は前提が成立しない。
> **この節の行番号は P-1〜P-9 の修正で 40〜50 行ずれており、純粋トグルの実数も 23 ではなく 25。
> 以下の数字を根拠に判断できる状態ではない。**
>
> **(a) トグル**: `def set(Enable|Disable)\w+` は 58 本、うち純粋な形は 25 本で 33 本は副作用あり。
> 決定的なのは、**下の「対応方針」が挙げる実害がこの修正では直らない**こと。
> 「ペアが非対称に育っており、コピペ元を間違えると副作用が片側だけ落ちる」で挙げた
> `setEnableTranslation` / `setDisableTranslation` のようなペアは、定義上すべて
> 「副作用あり」側＝**テーブルに入らない 33 本**である。純粋な 25 本を畳んでも
> 1 行も変わらない。むしろトグルが「テーブル側 25 本」と「手書き 33 本」に二分され、
> 新規追加時にどちらの流儀に従うかの判断が 1 つ増える。
>
> **(b) 数値セッター**: OBS 系 5 本は確かに均質だが、**呼び忘れは実際に起きていない**
> （5 本とも `_pushObsBrowserSourceSettings()` を呼んでおり、呼んでいない
> `setObsBrowserSourcePort` は送信 dict に port が含まれないので正しい。現時点でコピペ精度 100%）。
> 残る 7 本は ErrorCode もメッセージも後処理も個別で、均質なのは OBS の 5 本だけ。
> 実際に呼び忘れが起きた時点で、その 1 箇所を直すか、そのとき初めて畳むのが正しい順序。
>
> **(c) 委譲**: **S-1 と同じ失敗の形。** ただし S-1 と 1 点だけ条件が違い、5 エンジンは
> `engine_key.removesuffix("_API")` で導出できるので `controller.py` 側に名前テーブルは要らない。
> それでも着手しない理由は 3 つ:
> 1. **列挙は消えず `mainloop.py` に移るだけ。** `mainloop.py:298-303` が
>    `getPlamoModelList` / `getPlamoModel` / `setPlamoModel` / `getPlamoAuthKey` /
>    `setPlamoAuthKey` / `delPlamoAuthKey` を 1 行ずつ束縛しており、これが 5 エンジン分ある。
>    30 個の名前は結局 30 行として残る。S-1 の教訓「名前・文字列・テーブルのどこかに
>    同じ列挙が残るなら消えていない」に、形を変えて該当する。
> 2. **払う対価は S-1 と同一。** `grep "def getPlamoAuthKey"` が引けなくなり、
>    S-1 の巻き戻しで「払ったコストに対して何も買えていなかった」と結論した
>    `setattr` 機構をもう 1 セット持つことになる。
> 3. **導出規則が誰にも検証されていない仮定になる。** 隣の `getDeepLAuthKey` と
>    `setDeeplAuthKey` は**大文字小文字が揃っていない**。DeepL はレジストリ外なので
>    今回は当たらないが、「エンジン名からメソッド名が機械導出できる」という規則は
>    このファイルで実際には守られていない。規則に乗せた瞬間、次に追加する人が
>    命名を外したときに静かに 404 になる。
>
> **3 系統に共通して、着手根拠となる「実際に起きた事故」が存在しない。**
> 得られるのは行数削減のみで、それは着手条件を満たさない。

#### 元の指摘（参考）— S-3 機械的に集約できるパターンが 3 系統残っている
...
### ⛔ S-4 `models/obs` だけが config を直接 import している — **着手しない**（2026-09-15 のレビューで判定）

> **判定: 下の「対応方針」を実装すると挙動が退行する。**
>
> `_build_overlay_html()` は起動時ではなく **`do_GET` の中（`obs_browser_source_server.py:312`）で
> リクエストのたびに**呼ばれ、config を毎回読み直している。レスポンスヘッダも `Cache-Control: no-store`。
> つまり現状は「設定変更 → 開いているページへ WebSocket で live 反映」に加えて
> 「OBS がページを再読込 → **最新の config から HTML が再生成される**」が成立している。
>
> 起動時注入にすると後者が壊れ、**OBS Browser Source を Refresh した瞬間に
> 起動時点の設定へ巻き戻る**。ユーザーが次に設定を触るまで直らない。
>
> さらに、下の「対応方針」が挙げるもう 1 つの実害「`models/obs` の単体テストを書くとき
> config シングルトンを巻き込む」も**実在しない**。`test/test_websocket_auth_and_bind_guard.py:319,324`
> が既に `_build_overlay_html(ws_token="...")` を config の準備なしに直接呼んでテストしている。
>
> 正しくやるなら「server が可変の settings dict を持ち、`_pushObsBrowserSourceSettings()` から
> `updateSettings()` も呼ぶ」形になるが、これは**同期経路が 2 本になる**ので 30 行では済まず、
> ユーザーに見える利益はゼロ。`models/` の config 非依存という規則の純度のためだけに
> 実動作のリスクを買うことになる。
>
> **代案（未実施・やるなら 2 行）**: `obs_browser_source_server.py` の import 直上に
> 「リクエストごとに config を読み直す必要があるため、ここだけ意図的に config を直接見ている」
> と書く。次の人が踏襲する事故も、規則が曖昧なまま残る問題も、これで解消する。

#### 元の指摘（参考）— S-4 `models/obs` だけが config を直接 import している
...
### 🟢 S-5 `device_manager.py` のコメントが現在のコードと矛盾している（`09cfe48a`）
...
### ⛔ 前回項目 36「models の config 非依存化」— 残り 1 ファイルだが、それは直さない

実測の結果、残っているのは 1 ファイル・9 行のみ。大規模構造変更として構える必要はない。
**ただし当初「S-4 で完了」としたのは誤りで、S-4 は実装すると挙動が退行するため着手しない**（S-4 参照）。
`models/obs` の config 直参照は「リクエストごとに読み直す必要がある」という実装上の理由があり、
規則の例外としてコメントで明示するのが正しい扱い。

### ⛔ `Controller` のドメイン分割 — 前回の結論を維持

分割（コンポジション / ミックスイン）は依然として投資対効果が薄い。
なお当初ここに「S-1 + S-3 で controller.py は約 3900 行になる」と書いていたが、
**S-1 は巻き戻し、S-3 は着手しないと判定した**ため、この前提は成立しない。
`controller.py` は約 4940 行のままである。分割しない結論自体は変わらない。
...
| 項目 | 結果 |
|---|---|
| L-1 PyAutoGUI 排除 | 🟢 ペースト動作を確認。`paste: Ctrl+V の送出に失敗した` は 0 件 |
| P-10 安全弁の測り違い | 🟢 `accumulated` と `bytes` が一致（7.10s/227328、14.11s/451584、16.93s/541696。いずれも ÷32000 で検算一致） |
| 全体の安定性 | 🟢 3 セッション合計 約 2 時間、エラー / 例外 / traceback **0 件**、バーストなし |

### ⬜ P-8（VAD 経路の畳み込み）は未検証のまま締めた

`[VAD-merge] accumulate` が Google 区間で 1 件も出ず、**畳み込みの分岐が一度も走らなかった**。
条件は「VAD 有効 + Google + キューにバックログ」の 3 つ同時。

負荷を上げて（3 言語 = ASR が 3 回直列、1 回あたり 3.7〜5.9 秒）試したが、
`max_duration` セグメントの到着間隔が約 7 秒あるため、**次が届く前に ASR が終わってしまう**。
通常利用でこの状態に入るのは、ASR がセグメント間隔より遅くなったときだけ。

**打ち切り判断**: 再現条件が通常利用の範囲外であり、回帰テストで両経路の契約
（非 VAD と同じく `call_count == 1` に畳まれる）は固定済み。実機で追うコストに見合わない。

> なお 2026-09-16 の 1 本目のログで `accumulate` が 6 件出たが、これは**誤読しかけた**。
> 同セッション中にエンジンが Google → Whisper に切り替わっており、6 件はすべて Whisper 区間。
> `accumulate` は「Google でない」場合にも出る（`elif is_google and audio_queue.empty()` の
> `else` 側）ため、**畳み込みが走った証拠にはならない**。
> ログの 1 行だけを見て判定せず、同時刻の `[ASR-stats]` でエンジンを確認すること。

### ⚠️ 新たに数字で出た問題: Google + 多言語で認識失敗率が激増

今回の変更とは無関係の既存挙動だが、M-2 の計測によって初めて数値化された。

| 条件 | ASR 所要 | 成功率 |
|---|---|---|
| Google 1 言語（`calls=1/1`） | 約 500〜1400ms | **100%**（23/23） |
| Whisper 1 言語 | 約 1000〜1500ms | **100%**（13/13） |
| **Google 3 言語（`calls=3/3`）** | **3700〜5900ms** | **40%**（10/25） |

`[ASR-error]` は 0 件なので、ネットワークエラーではなく `UnknownValueError`（認識結果 0 件）。
終盤は 7 連続失敗。約 2 分間に 3 言語 × 25 回 = **75 リクエスト**を
Google の無料・非公式エンドポイントへ投げており、絞り込みを受けている可能性が高い。

**これは M-2 で「ASR の並列化を見送る」と判断した根拠を強化する。**
当時の根拠は「同セッションで ASR 失敗 7/22」だったが、今回はより悪い。
直列ですら 40% まで落ちるエンドポイントに同時リクエストを 3 倍にするのは悪化方向であり、
**並列化は引き続きやらない**。

**レイテンシを縮める実効的な手段はコードではなく設定**（候補言語を減らす）。
3 → 1 で ASR が約 1/4、かつ成功率が 40% → 100%。
コードで削れる余地より桁違いに大きい。
...
### 🟢 L-1 PyAutoGUI が GPLv3+ の依存を引き込んでいる（`1eb8e3ef`）

VRCT 本体は MIT。`PyAutoGUI==0.9.54` 自体は BSD だが、依存の **MouseInfo は GPLv3+**
（PyMsgBox も classifier は GPLv3+ で、上流のメタデータ自体が矛盾している）。
PyAutoGUI の `__init__.py` は両方を **import 時に**ロードするため、配布物に GPLv3 が入る。

`src-python` 全体で pyautogui を使っていたのは `clipboard.py` の 1 箇所だけで、
中身は実質 `pyautogui.hotkey('ctrl','v')` の 1 行。**PyAutoGUI の Windows 実装が
呼んでいるのも同じ `user32.keybd_event`** なので、ctypes 直呼びに置き換えた。
`clipboard.py` は `ctypes.WinDLL('user32')` を既にロード済みで、追加の足場は不要。

- `paste_via_pyautogui` → `paste_via_ctrl_v` にリネーム（pyautogui を使わなくなり名前が嘘になるため）
- `requirements.txt` / `requirements_cuda.txt` から削除
- `keybd_event` の呼び出し順と、**V の送出が失敗しても Ctrl を必ず離すこと**
  （押しっぱなしだと以後の操作が全て Ctrl 付きになる）を守るテストを追加

**未検証**: 他プロセスのウィンドウへの実際のキー送出は権限依存で、ユニットテストでは
検出できない（`translators` 並列化で一度踏んだのと同じ性質）。**実機でのペースト動作確認が必要。**

### ⛔ L-2 `translators`（GPLv3）の排除 — リファクタではなく製品判断

`translators` は GPLv3。使用箇所は `translation_translator.py` の 3 エンジン
（**Google / Bing / Papago**）で、これは**無料・無設定で使える唯一のエンジン群**である
（他は全て認証キーかローカルモデルが要る）。排除は機能削除に直結する。

さらに `requirements.txt` はフォーク `misyaguziya/translators@f3697bd` に固定されており、
これは **P-5（翻訳並列化）の前提**として `Tse.session` を `threading.local()` へ分離した改修そのもの。
排除すれば P-5 の成立条件が消える。

代替の見積もり:
- Google: 無料エンドポイントを自前で叩くのは現実的（40 行程度）
- Bing / Papago: トークン / セッションのスクレイピングが必要で、
  上流が壊すたびに追随する保守負債を丸ごと引き受けることになる

**オーナーに問うべきは「行数」でも「構造」でもなく、`Bing` / `Papago` を落としてよいかの一点。**
MIT で配布するバイナリに GPLv3 をリンクしている法務上の露出は実在するが、
その解消コストは**ユーザーに見える機能削除**の可否に依存する。
（別プロジェクトとして MIT 実装を進め、完成次第差し替える方針は維持。）

---

## レビュー結果（2026-09-15、レビュアー 3 名）

S-1 の巻き戻しを受け、**残タスクが同じ失敗を繰り返さないか**を着手前に判定するため、
独立した 3 つの観点でレビューを実施した。

| 観点 | 結果 |
|---|---|
| 実行時の正しさ / 並行性 | **要修正 1 件（高）** → `475e3634` で修正。他は問題なし |
| 割に合っているか（S-1 と同じ形が残っていないか） | **指摘ゼロ** |
| 残タスクの事前判定 | **7 件中 4 件をクローズ**（S-3 / S-4 / P-6 / L-2） |

**得られたもの**:

1. **P-8 の VAD 経路適用漏れ**（上記）。しかも doc 側に「P-8 / P-9 とも是正済み」と
   **未確認のまま書いていた**。レビューが無ければ「直っている」と誤認したままだった。
2. **S-4 と P-6 は、実装していれば S-1 と同じ巻き戻しになっていた**。どちらも
   「対応方針」が現物と噛み合っていない（S-4 は `do_GET` 内でのリクエスト時生成を見落とし、
   P-6 は `_stop_monitoring` の読み取り位置を見落としていた）。
   **レビュー段階で実際のコードを読むだけで、実装前に判明した。**
3. **S-2 は当初の仕様のままでは成立しなかった**（偽陽性 15 件）。

**教訓**: レビューの価値は「実装の粗を探すこと」より、
**「対応方針が現物と噛み合っているかを、実装前に現物で確かめること」**にあった。
実装してから気づく形の項目（S-4 / P-6）を 2 件、着手前に止められた。

**計測されたもう 1 つの傾向**: 今回クローズした 4 件はいずれも
「着手根拠が *実際に起きた事故* ではなく *将来困るかも* だった」項目である。
S-1 の教訓（行数削減それ自体は成果ではない）は、着手条件の言い換えとして機能した。
